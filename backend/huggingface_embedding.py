...
# Fetching only one CVE from NVD API with API key
def fetch_single_cve():
    ...
    if nvd_api_key:
        ...
        print("Fetching CVE from NVD API (with API key)")
    else:
        print("Fetching CVE from NVD API (without API key)")
...
"""
    ...
    # Create metadata (vulnerability attributes excluding the embeddings above)
    metadata = {
        "cve_id": cve['id'],
...
# Embedding and storing in ChromaDB using HuggingFace
def embed_and_store(document):
    """Embedding the document using HuggingFace and store in ChromaDB"""
    
    print("Initializing HuggingFace embeddings (local, using M3)...")
    print("Note: First time will download model (~400MB)...")
    ...
    vectorstore = Chroma.from_documents(
        ...
        persist_directory="./chroma_db_hf",  # Different directory to avoid conflicts
        collection_name="cve_collection_hf"
    )
...
if __name__ == "__main__":
    print("="*60)
    print("TESTING HUGGINGFACE EMBEDDING WITH LLM")
    print("="*60 + "\n")
